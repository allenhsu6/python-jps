...
class JPS():
    ...
    def AStar(self, node):
        ...
        for i in range(len(neighbors)):
            ...
            neighborNode = self.grid.getNodeAt(jx, jy)
            ...
            if not neighborNode.opened or ng < neighborNode.g:

                neighborNode.g = ng
                neighborNode.h = neighborNode.h or self.heuristic(abs(jx - endX), abs(jy - endY))
                neighborNode.f = neighborNode.g + neighborNode.h
                neighborNode.parent = node

                if not neighborNode.opened:
                    heapq.heappush(self.openList, (neighborNode.g - neighborNode.f, neighborNode))
                    neighborNode.opened = True
                else:   #
                    for i in range(len(self.openList)):
                        if self.openList[i][1] == neighborNode:
                            self.openList[i] = (neighborNode.g - neighborNode.f, neighborNode)
